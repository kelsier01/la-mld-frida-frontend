<template>
    <ion-card class="product-card">
      <div class="image-container">
        <ion-img :src="`${IMAGES_URL}${producto.imagenes[0].url}`" alt="Imagen del producto" class="product-image" />
      </div>
      <ion-card-header>
        <ion-card-title>{{producto.marca.nombre}} - {{ producto.nombre }}</ion-card-title>
        <ion-card-subtitle>{{ producto.categoria.nombre }}</ion-card-subtitle>
      </ion-card-header>
      <ion-card-content class="card-content">
        <p><strong>Código:</strong> {{ producto.codigo }}</p>
        <p><strong>Bodega:</strong> {{ producto.bodegas[0].bodega.nombre }}</p>
