      ...
      <ion-grid>
        <ion-row>
          ...
          <ion-col size="6">
            ...
            <ion-text v-else color="danger"> No hay stock disponible </ion-text>
          </ion-col>
        </ion-row>
        <ion-row>
          <ion-col>
...
const actualizarStockDisponible = () => {
  ...
  if (bodegaSeleccionada.value === 4) {
    stockDisponible.value = 1; // Valor especial para indicar que no hay límite
    return;
  }

  // Para otras bodegas, mantener el comportamiento original
  const bodega = props.producto.Producto.ProductoBodegas.find(
    (b) => b.bodegas_id === bodegaSeleccionada.value
  );
  if (bodega) {
...
onMounted(() => {
  if (props.producto.Producto.ProductoBodegas.length > 0) {
    ...
    const bodegaPreferida = props.producto.Producto.ProductoBodegas.find(
      (b) => b.bodegas_id === 4
    );
    ...
    if (bodegaPreferida) {
      ...
      bodegaSeleccionada.value =
        props.producto.bodegas_id ||
        props.producto.Producto.ProductoBodegas[0]?.bodegas_id ||
        0;
    }
    ...
    precioVenta.value =
      props.producto.precio_venta || props.producto.Producto.precio_venta;
    precioCompra.value =
      props.producto.precio_compra_usd ||
      props.producto.Producto.Precio_compra_usd;
    actualizarStockDisponible();
  }
});
</script>
