<template>
  <ion-header>
    <IonItem>
      ...
      >
        <IonSelectOption value="0"> Todas las regiones </IonSelectOption>
        <IonSelectOption
  ...
  <ion-content class="ion-padding" @ionInfinite="loadMorePedidos">
    ...
    <div v-else>
      <ion-card v-if="pedidosFiltrados.length === 0 && !loading">
      ...
      <ion-infinite-scroll @ionInfinite="loadMorePedidos" threshold="5px">
...
const pedidosFiltrados = computed(() => {
  if (!fecha_envio.value) {
    return pedidos.value;
  }

  const fechaSeleccionada = new Date(fecha_envio.value)
    .toISOString()
    .split("T")[0];

  return pedidos.value.filter((pedido) => {
    // Asumiendo que el pedido tiene una propiedad fecha_envio
    const fechaPedido = pedido.fecha_entrega
  ...
  try {
    ...
    if (response.pedidos) {
      const pedidosFiltrados = response.pedidos.filter(
        (pedido: Pedido) =>
          pedido.guia_despacho_id !== null &&
          (pedido.estado_pedidos_id === 3 ||
            pedido.estado_pedidos_id === LISTO_PARA_DESPACHO)
      );
...
const confirmarYRecepcionar = async () => {
  ...
  try {
    ...
    const pedidosPorCliente = pedidosSeleccionados.value.reduce<
      Record<number, Pedido[]>
    >((groups, pedido) => {
    ...
    for (const clienteId in pedidosPorCliente) {
      ...
      const comprobanteResponse =
        await comprobanteVentaService.postComprobanteVenta({
